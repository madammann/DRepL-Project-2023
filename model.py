...
class PolicyHead(tf.keras.Model):
    '''
    The final part of either model, a fully-connected network processing an embedding into a (4,) continous output.
    '''
    
    def __init__(self, layers=1, k_init='glorot_uniform', b_init='zeros'):
        '''
        Constructor method for the PolicyHead model part.
    ...
    def __call__(self, x):
        ...
        :param x (tf.Tensor): The input embedding to the model PolicyHead in shape (batch, n) for n feature dimension.
...
class DeepQNetwork(tf.keras.Model):
    def __init__(self, optimizer, visual=False, rgb=True, cnn_depth=1, mlp_layers=1, head_layers=1, filters=1, kernel_size=3, k_init='glorot_uniform', b_init='zeros'):
        ...
        #we build the model
        self.model_layers = [PolicyHead(layers=head_layers, k_init=k_init, b_init=b_init)]
